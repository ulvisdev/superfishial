...
public class DialogueVertexAnimator {
    ...
    private readonly float voicePitch;
    ...
    public IEnumerator AnimateTextIn(List<DialogueCommand> commands, string processedMessage, Action onFinish, AudioClip _audioClip, float _voicePitch = 1, bool _randomPitch = false) {
        textAnimating = true;
            ...
            if (ShouldShowNextCharacter(secondsPerCharacter, timeOfLastCharacter)) {
                if (visableCharacterIndex <= charCount) {
                    ...
                    if (visableCharacterIndex < charCount && ShouldShowNextCharacter(secondsPerCharacter, timeOfLastCharacter)) {
                        ...
                        PlayDialogueSound(_audioClip, _voicePitch, _randomPitch);
    ...
    private void PlayDialogueSound(AudioClip _audioClip, float _voicePitch = 1, bool _randomPitch = false) {
        if (_audioClip != null) SoundEffectManager.PlayVoice(audioClip, voicePitch, randomPitch);
    }
